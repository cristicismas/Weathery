...
exports.getCurrentWeather = async function(req, res, next) {
  try {
    request(`https://api.apixu.com/v1/current.json?key=${process.env.APIXU_API_KEY}&q=${req.params.query}`, function(err, response, body) {
      ...
      if (err) {
        ...
        const currentDay = body.current;

        const data = {
          city: currentLocation.name,
          region: currentLocation.region,
          country: currentLocation.country,
          temp_c: currentDay.temp_c,
          temp_f: currentDay.temp_f,
          wind_kmh: currentDay.wind_kmh,
          wind_mph: currentDay.wind_mph,
          precip_mm: currentDay.precip_mm,
          precip_in: currentDay.precip_in,
